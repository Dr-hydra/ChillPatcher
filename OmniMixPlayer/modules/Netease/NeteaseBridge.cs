    ...
    public class NeteaseBridge
    {
        ...
        public long CreatePcmStream(long songId, Quality quality = Quality.ExHigh)
        {
            ...
            try
            {
                ...
                {
                    _logger.LogInformation($"[NeteaseBridge] Created PCM stream {streamId} for song {songId}");
                }
                return streamId;
            }
            catch (Exception ex)
        ...
        public void ClosePcmStream(long streamId)
        {
            try
            {
                ...
                _logger.LogInformation($"[NeteaseBridge] Closed PCM stream {streamId}");
            }
            catch (Exception ex)
            {
        ...
        public int SeekPcmStream(long streamId, long frameIndex)
        {
            try
            {
                ...
                if (result == 0)
                {
                    _logger.LogInformation($"[NeteaseBridge] Seeked PCM stream {streamId} to frame {frameIndex}");
                }
                else if (result == -3)
                {
                    _logger.LogInformation($"[NeteaseBridge] Pending seek set for PCM stream {streamId} to frame {frameIndex} (waiting for cache)");
                }
                else if (result == -2)
                {
