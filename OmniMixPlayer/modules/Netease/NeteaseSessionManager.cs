...
namespace OmniMixPlayer.Module.Netease
{
    ...
    public class NeteaseSessionManager
    {
        ...
        public NeteaseSessionManager(NeteaseBridge bridge, ILogger logger)
        {
            _bridge = bridge;
            _logger = logger;
        }
        ...
        {
            _logger.LogInformation("[NeteaseSession] Validating session...");
            ...
            switch (result)
            {
                case NeteaseBridge.RefreshLoginResult.Success:
                    ...
                    _logger.LogInformation($"[NeteaseSession] Session valid. User: {UserInfo?.Nickname}, VipType: {UserInfo?.VipType}");
                    return true;
        ...
        {
            _logger.LogInformation($"[NeteaseSession] Trial detected for song {songId}, attempting recovery...");
            ...
            switch (refreshResult)
            {
                ...
                case NeteaseBridge.RefreshLoginResult.Success:
                    UpdateUserInfo();
                    SetState(SessionState.LoggedIn);
                    _logger.LogInformation("[NeteaseSession] Silent refresh succeeded, retrying song URL...");
                ...
                case NeteaseBridge.RefreshLoginResult.AuthFailed:
                    ...
                    {
                        _logger.LogInformation("[NeteaseSession] Trial recovery cancelled (song changed?)");
                        return TrialRecoveryResult.NetworkError;
                    }

                    _logger.LogInformation("[NeteaseSession] QR login succeeded, retrying song URL...");
                    return await RetryGetSongUrl(songId, quality, cancellationToken);
        ...
        public void TriggerQRLogin()
        {
            ...
            _logger.LogInformation("[NeteaseSession] QR login triggered");
        }
        ...
        public void Logout()
        {
            _logger.LogInformation("[NeteaseSession] Logging out...");
            var result = _bridge.Logout();
            UserInfo = null;
            RecoveredSongUrl = null;
            SetState(SessionState.Expired);
            _logger.LogInformation($"[NeteaseSession] Logout completed: {(result ? "success" : "failed")}");
        }
        ...
        public void NotifyLoginSuccess()
        {
            ...
            _logger.LogInformation($"[NeteaseSession] Login success notified. User: {UserInfo?.Nickname}");
        }
            ...
            CancellationToken cancellationToken)
        {
            ...
            _logger.LogInformation($"[NeteaseSession] Song {songId} recovered successfully");
        ...
        private void SetState(SessionState newState)
        {
            ...
            var oldState = State;
            State = newState;
            _logger.LogInformation($"[NeteaseSession] State: {oldState} -> {newState}");
