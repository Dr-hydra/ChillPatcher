...
namespace OmniMixPlayer.Module.Spotify
{
    ...
    public class SpotifyBridge : IDisposable
    {
        ...
        public SpotifyBridge(string clientId, string dataPath, ILogger logger)
        {
            _clientId = clientId;
            _dataPath = dataPath;
            _logger = logger;
        ...
        public void LoadSession()
        {
            ...
            try
            {
                ...
                _logger.LogInformation($"Loaded Spotify session for {_session?.DisplayName ?? "unknown"}");
            }
            catch (Exception ex)
            {
        ...
        {
            _logger.LogInformation("Fetching user playlists...");
            ...
            while (true)
            {
                ...
                _logger.LogInformation($"  Fetched {all.Count}/{page.Total} playlists");
                if (string.IsNullOrEmpty(page.Next)) break;
                offset += limit;
            }

            _logger.LogInformation($"Total playlists fetched: {all.Count}");
            return all;
        }
        ...
        {
            _logger.LogInformation($"Fetching tracks for playlist {playlistId}...");
