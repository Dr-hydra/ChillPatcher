...
#Region "基础"
    ...
    Public Sub New()
        ...
        '切换到首页
        If IsOmniMixGui Then
            If Not IsNothing(FrmOmniMixLeft.Parent) Then FrmOmniMixLeft.SetValue(ContentPresenter.ContentProperty, Nothing)
            If Not IsNothing(FrmOmniMixHome.Parent) Then FrmOmniMixHome.SetValue(ContentPresenter.ContentProperty, Nothing)
        ...
        End If
        '模式提醒
        If BuildType = BuildTypes.Debug Then Hint(If(IsOmniMixGui, "[开发者模式] OmniMix GUI 正在使用迁移版 WPF UI 框架运行。", "[开发者模式] PCL 正以开发者模式运行，这可能会造成严重的性能下降，请务必立即向开发者反馈此问题！"), HintType.Red)
        If ModeDebug Then Hint(If(IsOmniMixGui, "[调试模式] OmniMix GUI 正以调试模式运行，这可能会导致性能下降。", "[调试模式] PCL 正以调试模式运行，这可能会导致性能下降，若无必要请不要开启！"))
        '尽早执行的加载池
        If Not IsOmniMixGui Then McFolderListLoader.Start(0) '为了让下载已存在文件检测可以正常运行，必须跑一次；为了让启动按钮尽快可用，需要尽早执行；为了与 PageLaunchLeft 联动，需要为 0 而不是 GetUuid
    ...
    Private Sub FormMain_Loaded(sender As Object, e As RoutedEventArgs) Handles Me.Loaded
        ...
        Catch ex As Exception '修复 #2019
            ...
            Width = MinWidth + 100
        End Try
        If IsOmniMixGui Then Height = Math.Max(Height, 650)
        'MinHeight = 50
        'MinWidth = 50
        Topmost = False
        ...
        TimerMainStart()
        If IsOmniMixGui Then
            Logger.Info("OmniMix GUI 已启动，旧桌面业务加载池已跳过。")
...
#Region "自定义窗口"
    ...
    Public Sub UpdateBackgroundAndTitleBar()
        ...
        End If
        PanTitleSelect.Visibility = Visibility.Visible
        LabOmniMixConnectionStatus.Visibility = Visibility.Collapsed
        '标题栏显示类型
    ...
    Public Sub BtnExtraShutdown_Click() Handles BtnExtraShutdown.Click
        Try
            ...
            For Each Watcher In McWatcherList
                Watcher.Kill()
            Next
            Hint("已停止运行中的后台任务！", HintType.Green)
        Catch ex As Exception
            Logger.Error(ex, "强制停止后台任务失败")
        End Try
    End Sub
