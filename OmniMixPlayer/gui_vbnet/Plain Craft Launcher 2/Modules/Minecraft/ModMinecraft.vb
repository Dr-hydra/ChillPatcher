...
#Region "将版本分类到各个卡片"
        Try
            ...
            '确认最新版本，若为快照则加入常用列表
            Dim LargestInstance As McInstance = MeloongCore.Extensions.EnumerableExtensions.MaxBy(
                InstanceList.Where(Function(v) v.State = McInstanceState.Original OrElse v.State = McInstanceState.Snapshot),
                Function(v) v.ReleaseTime)
...
#Region "支持库文件（Library）"
    ...
    Public Function McLibNetFilesFromTokens(Libs As List(Of McLibEntry), Optional CustomMcFolder As String = Nothing) As List(Of NetFile)
        ...
        Next
        '去重并返回
        Return MeloongCore.Extensions.EnumerableExtensions.DistinctBy(Result, Function(a) a.LocalPath).ToList
    End Function
    ''' <summary>
    ''' 获取对应的支持库文件地址。
    ''' </summary>
