    ...
    Private Sub MusicListInit(ForceReload As Boolean, Optional PreventFirst As String = Nothing)
        ...
        Try
            ...
            If MusicAllList Is Nothing Then
                ...
                For Each File In DirectoryUtils.GetFiles(PathExeFolder & "OmniMixPlayer\Musics\")
...
#Enable Warning BC40008 '类型或成员已过时
        ...
        Catch ex As Exception
            ...
            If TypeOf ex Is NAudio.MmException AndAlso ex.Message.Contains("AlreadyAllocated") Then
                Hint("你的音频设备正被其他程序占用。请在关闭占用的程序后重启 OmniMix Player，才能恢复音乐播放功能！", HintType.Red)
                Thread.Sleep(1000000000)
            End If
            If TypeOf ex Is NAudio.MmException AndAlso (ex.Message.Contains("NoDriver") OrElse ex.Message.Contains("BadDeviceId")) Then
                Hint("由于音频设备变更，音乐播放功能在重启 OmniMix Player 后才能恢复！", HintType.Red)
                Thread.Sleep(1000000000)
            End If
            If Not (MusicCurrent.EndsWithF(".wav", True) OrElse MusicCurrent.EndsWithF(".mp3", True) OrElse MusicCurrent.EndsWithF(".flac", True)) OrElse
                ...
                Hint("播放音乐失败（" & PathUtils.GetLastPart(MusicCurrent) & "）：OmniMix Player 可能不支持此音乐格式，请将格式转换为 .wav、.mp3 或 .flac 后再试", HintType.Red)
            Else
