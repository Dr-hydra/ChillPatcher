...
#Region "基础请求引擎"
    ...
    Private Function DNSLookup(Request As HttpRequestMessage, CancelToken As CancellationToken) As String
        ...
        End If
        '选择可靠度最高的 IP
        Dim Target As IPAddress = OmniMixCore.Extensions.EnumerableExtensions.MaxBy(Candidates, Function(i) Reliabilities(i))
...
#End Region
        ...
        Public Overrides Sub Start(Optional Input As Object = Nothing, Optional IsForceRestart As Boolean = False)
            ...
            '去重
            Files = New ConcurrentList(Of NetFile)(OmniMixCore.Extensions.EnumerableExtensions.DistinctBy(Files, Function(a) a.LocalPath))
            '设置剩余文件数
            ...
            Sub()
                Try
                    ...
                    '获取可复用文件查找目录。旧 Minecraft 实例列表已移除，仅保留官方默认目录兜底。
        ...
        Private Function CheckExistingFile(FolderList As List(Of String), VersionFolders As List(Of String), File As NetFile) As String
            ...
            If Not TypeIndexes.Any Then
                ...
                End If
            End If
            Dim Type = OmniMixCore.Extensions.EnumerableExtensions.MaxBy(TypeIndexes, Function(kv) kv.Item2).FolderName.TrimStart("\"c)
            '根据类别进行查找
